...
function renderCart(data) {
  ...
  for (var i = 0; i < data.items.length; i++) {
    ...
    if (atMaxStock) {
      html +=
        '  <p class="stock-warning">Max stock reached (' +
        item.stock +
        " available)</p>";
    }
    html += "</div>";
    ...
    html +=
      ...
      item.product_id +
      ', \'decrement\')" title="Decrease quantity">−</button>';
    ...
    html +=
      '<button class="remove-btn" onclick="updateQuantity(' +
      item.product_id +
      ', \'remove\')" title="Remove item">✕ Remove</button>';
...
function showNotification(message, type) {
  ...
  notification.style.cssText =
    "position:fixed;top:1rem;right:1rem;padding:0.75rem 1.25rem;border-radius:6px;font-size:0.9rem;font-weight:500;z-index:9999;animation:slideIn 0.3s ease;";
